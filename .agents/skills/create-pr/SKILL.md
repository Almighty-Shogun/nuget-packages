---
name: create-pr
description: Create a GitHub pull request for Almighty-Shogun/nuget-packages from an optional branch name. Use when the user invokes $create-pr, optionally with a fix/* or feat/* branch parameter, asks to reuse the current fix/* or feat/* branch, infer a branch name for current changes, commit selected files through the user-level $commit skill, generate a PR title/body in the repository's personal style, and create the PR only after explicit approval.
---

# Create PR

Create a focused pull request for `Almighty-Shogun/nuget-packages`.

The user may invoke this skill with or without a branch parameter:

```text
$create-pr
$create-pr fix/jwt-auth-registration
```
...
- Use the provided `fix/*` or `feat/*` branch when present.
- Reuse the current `fix/*` or `feat/*` branch when no branch is provided.
- Infer `fix/<slug>` for bug fixes, regressions, broken behavior, docs corrections, or reliability fixes.
- Infer `feat/<slug>` for new user-facing package behavior or new documentation/product capabilities.
- Reject names with whitespace, `..`, `@{`, `//`, leading `-`, trailing `/`, or `.lock`.
- Reject protected names such as `main`, `master`, `development`, or `release`.
...
Useful repository groups:

- Package source: `packages/<PackageName>/**`
- Package docs: `docs/<package-slug>/**`
- VitePress config/theme: `docs/.vitepress/**`
- CI/release: `.github/workflows/**`
- Agent instructions/skills: `AGENTS.md`, `docs/AGENTS.md`, `.agents/skills/**`
...
After the branch exists and the intended file scope is clear, invoke the user-level `$commit` skill. Let that skill handle staging, commit grouping, verification, commit creation, and push confirmation.
...
- Keep the title direct and short, for example `Bugfix: JWT auth registration`.
...
```text
/tmp/nuget-packages-pr-<branch-slug>.md
```
