...
Validates that an email address is not already assigned to another credential user. [`RegisterRequest`](../requests/register-request) and [`CreateUserRequest`](../requests/create-user-request) use this attribute so user creation can reject duplicate email addresses before the database unique index is hit.
