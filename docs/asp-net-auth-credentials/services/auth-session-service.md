...
The session expires one refresh window from now, or at [`AbsoluteSessionLifetimeDays`](../configuration) when that ceiling falls sooner, so a client that never refreshes cannot hold a usable token past it.
...
Matches the submitted token against a live session, rotates it, refreshes the recorded request metadata, and returns a new access token. The new expiry is held to the same [`AbsoluteSessionLifetimeDays`](../configuration) ceiling the session was opened under, so refreshing extends a session but cannot keep it alive forever.
...
Throws [`InvalidSessionException`](../exceptions) when the token matches no usable session, whether unknown, expired, revoked, or scoped to a different application, and when the session has reached the absolute lifetime measured from when it was created. Two refreshes racing on one session are settled by the database, whether by the concurrency token leaving the losing rotation matching no row or by a server that rejects it outright for having read the row before the winner wrote it, and the one that loses is refused the same way. A disabled or locked-out account is refused with [`AccountDisabledException`](../exceptions) or [`AccountLockedException`](../exceptions), so deactivating a user takes effect on their next refresh rather than at the end of their access token.
