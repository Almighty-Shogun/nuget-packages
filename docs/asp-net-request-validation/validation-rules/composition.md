...
::: tip
There is no attribute equivalent for `AnyOf`. An `AnyOf<TRule>` attribute would only be another name for [`[CustomRule<TRule>]`](../custom-rules), because C# attributes cannot hold nested fluent rule builders or call other validation attributes as executable rules. Use fluent `AnyOf` when composing built-in rules, and `[CustomRule<TRule>]` when attribute-based code needs custom branching logic.
:::
