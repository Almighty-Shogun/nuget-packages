    ...
    - name: MaintenanceSettings
      ...
      fields:
          - name: MaintenancePath
            description: Request path that returns maintenance details while maintenance mode is enabled. Matched and emitted relative to the path base, so an application mounted under one keeps this value as written. Validated at startup, so a value carrying whitespace, a query string, or a fragment fails the host.
            type: string
            default: /maintenance
