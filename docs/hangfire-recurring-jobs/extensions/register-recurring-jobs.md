...
Registers the recurring job classes found in the scanned assemblies and the hosted service that hands their schedules to Hangfire when the host starts. A class is discovered when it implements [`IRecurringJob`](../types/recurring-job) and carries [`RecurringJobAttribute`](../attributes/recurring-job-attribute), and the [`RecurringJobs`](../configuration) section can then override what that attribute declares. Pair it with [`AddCustomHangfire`](./add-custom-hangfire), in either order, since neither reads what the other registered.
