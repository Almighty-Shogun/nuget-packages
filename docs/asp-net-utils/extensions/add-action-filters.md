...
Registers MVC controller services and adds the package `SessionContextFilter` as a global action filter. The filter captures request metadata before controller actions run and stores it in `HttpContext.Items` for later access through [`GetSessionContext`](./get-session-context).
